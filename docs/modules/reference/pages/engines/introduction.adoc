...
= Correlation Engines
:description: Learn how the correlation engines (DBSCAN clustering and LLM clustering) in OpenNMS's Architecture for Learning Enabled Correlation (ALEC) work.

Correlation engines are the underlying processes that drive ALEC.
The xref:engines/clustering.adoc[clustering] engine (DBSCAN) is implemented by default; an experimental xref:engines/llm-clustering.adoc[LLM clustering] engine is also available.
