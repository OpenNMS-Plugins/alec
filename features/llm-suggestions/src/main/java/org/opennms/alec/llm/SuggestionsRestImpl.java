...
public class SuggestionsRestImpl implements SuggestionsRest {
    ...
    private final LlmSituationHandler handler;
    ...
    public SuggestionsRestImpl(SuggestionStore store,
                               ...
                               LlmSituationHandler handler,
                               LlmConfigReader configReader) {
    ...
    private String resolveStorageKey(String longIdParam) {
        ...
        try {
            ...
            return maybe.map(LlmSituationHandler::keyFor).orElse(null);
    ...
    public Response reanalyze(String situationId) {
        ...
        Optional<LlmConfigReader.Config> config = configReader.read();
        if (config.isEmpty() || !config.get().isEnabled() || !config.get().hasApiKey()) {
            return Response.status(Response.Status.BAD_REQUEST)
                    .entity("LLM integration is not enabled — set an API key and enable on the configuration page")
                    .build();
        }
        ...
        Optional<SuggestionRecord> pending = store.get(LlmSituationHandler.keyFor(situation));
