...
public class SuggestionStoreTest {
    ...
    public void putPendingPersistsCorrectShape() throws Exception {
        store.putPending("sit-1", 1000L, "claude-sonnet-4-6");
        ...
        assertThat(node.get("model").asText(), equalTo("claude-sonnet-4-6"));
    }

    @Test
    public void putReadyCarriesArraysAndCompletedAt() {
        store.putReady("sit-2", 1000L, 1500L, "claude-sonnet-4-6",
    ...
    public void putFailedCarriesErrorAndEmptyArrays() {
        store.putFailed("sit-3", 1000L, 1200L, "claude-sonnet-4-6", "Timeout after 30s");
